            ...
            if (!empty($fetched_data['story_id'])) {
                ...
                if (!empty($fetched_data['story']) && !empty($fetched_data['story'][0])) {
                    $fetched_data['story'] = $fetched_data['story'][0];
                        ...
                        $is_worldmates = isset($_POST['use_gcm']) && $_POST['use_gcm'] == 'true' ||
                            isset($_GET['use_gcm']) && $_GET['use_gcm'] == 'true';
            
            if (!$is_worldmates && !empty($fetched_data['iv']) && !empty($fetched_data['tag'])) {
                // Подключаем crypto_helper
                $crypto_path = '/var/www/www-root/data/www/worldmates.club/api/v2/crypto_helper.php';
                ...
                if (class_exists('CryptoHelper')) {
                    try {
                        ...
                        if ($ecb_text !== false) {
                            $fetched_data['text'] = $ecb_text;
                        }
                    } catch (Exception $e) {
                        error_log("Wo_GetMessages GCM→ECB error: " . $e->getMessage());
                    }
                }
            }
...
function Wo_GetGroupMessages($args = array())
{
    ...
    if ($old && $offset && $offset > 0 && !$new) {
        ...
        $query_one = " SELECT m.*, 
                          ...
                          m.text_preview 
                   FROM " . T_MESSAGES . " AS m 
                   WHERE `group_id` = '$group_id' {$query_one} ";
    $sql_query_one = mysqli_query($sqlConnect, $query_one);
    $query_limit_from = mysqli_num_rows($sql_query_one) - 50;
    if ($query_limit_from < 1) {
        $query_limit_from = 0;
...
function Wo_GetPageMessages($args = array())
{
    ...
    if ($logged_user_id != $page_user_id) {
            $query_one = " SELECT m.*, 
                          ...
                          m.text_preview 
                   FROM " . T_MESSAGES . " AS m 
                   WHERE `page_id` = '$page_id' {$query_one} AND ((`from_id` = '$logged_user_id' AND `to_id` = '$page_user_id') OR (`from_id` = '$page_user_id' AND `to_id` = '$logged_user_id') ) ";
