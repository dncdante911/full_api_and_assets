...
$base_path = dirname(__DIR__, 2); // Подняться на 2 уровня от /xhr/ = корень сайта
$browser_compatibility_path = $base_path . '/api/v2/browser_compatibility.php';

if (file_exists($browser_compatibility_path)) {
    require_once($browser_compatibility_path);
}

error_log("=== MESSAGES.PHP DEBUG ===");
error_log("POST: " . print_r($_POST, true));
error_log("GET: " . print_r($_GET, true));
error_log("REQUEST URI: " . $_SERVER['REQUEST_URI']);

// Проверим Wo_GetMessages
$test_msg = Wo_GetMessages(array('user_id' => 1), 1);
error_log("Wo_GetMessages test: " . print_r($test_msg, true));

// Проверим поля первого сообщения
if (!empty($test_msg[0])) {
    error_log("Message fields:");
    error_log("- Has iv: " . (!empty($test_msg[0]['iv']) ? 'YES' : 'NO'));
    error_log("- Has tag: " . (!empty($test_msg[0]['tag']) ? 'YES' : 'NO'));
    error_log("- cipher_version: " . ($test_msg[0]['cipher_version'] ?? 'none'));
}
ini_set('error_log', '/var/www/www-root/data/www/worldmates.club/api/v2/logs/messages_final.log');
// === ГЛОБАЛЬНАЯ ФУНКЦИЯ КОНВЕРТАЦИИ ДЛЯ БРАУЗЕРА ===
function convertMessagesForBrowser(&$messages) {
    ...
    $crypto_path = dirname(__DIR__, 2) . '/crypto_helper.php';
    if (file_exists($crypto_path) && !class_exists('CryptoHelper')) {
    ...
    foreach ($messages as &$message) {
        ...
        $has_gcm = !empty($message['iv']) && 
                   !empty($message['tag']) && 
                   isset($message['cipher_version']) && 
                   $message['cipher_version'] == 2;
        
        $has_ecb = !empty($message['text_ecb']);
        $message_id = $message['id'] ?? 0;
        $timestamp = $message['time'] ?? time();
        
        // Если это GCM и мы в браузере - конвертируем в ECB
        if ($has_gcm && class_exists('CryptoHelper') && !$has_ecb) {
            try {
                // Конвертируем GCM → ECB
                $ecb_text = CryptoHelper::convertGCMtoECB(
                    $message['text'],
                    $timestamp,
                    $message['iv'],
                    $message['tag']
                ...
                if ($ecb_text !== false) {
                    ...
                    if ($message_id && isset($db) && $db) {
                        $db->where('id', $message_id)->update('Wo_Messages', array(
                            'text_ecb' => $ecb_text
                        ));
                    }
                }
                ...
                error_log("convertMessagesForBrowser error: " . $e->getMessage());
            }
        }
        
        // Если есть ECB версия, используем её
        if ($has_ecb) {
...
// === ФУНКЦИЯ ДЛЯ WORLDMATES (оставляем GCM) ===
function convertMessagesForWorldMates(&$messages) {
    if (empty($messages) || !is_array($messages)) {
        return;
    }
    ...
    foreach ($messages as &$message) {
        ...
        // Если есть только ECB, но нет GCM - конвертируем для WorldMates
        $has_gcm = !empty($message['iv']) && !empty($message['tag']);
        $has_ecb = !empty($message['text_ecb']);
        $timestamp = $message['time'] ?? time();
        
        if (!$has_gcm && $has_ecb && class_exists('CryptoHelper')) {
            try {
                $gcm_data = CryptoHelper::convertECBtoGCM($message['text_ecb'], $timestamp);
                if ($gcm_data !== false) {
                    $message['text'] = $gcm_data['text'];
                    $message['iv'] = $gcm_data['iv'];
                    $message['tag'] = $gcm_data['tag'];
                    $message['cipher_version'] = $gcm_data['cipher_version'];
                }
            } catch (Exception $e) {
    ...
    if ($s == 'get_user_messages') {
        if (!empty($_GET['user_id']) AND is_numeric($_GET['user_id']) AND $_GET['user_id'] > 0 && Wo_CheckMainSession($hash_id) === true) {
            ...
            $messages   = Wo_GetMessages(array(
             ...
             $is_worldmates = false;
            // Можно проверить по user-agent или другим признакам
            
            if (!empty($messages) && is_array($messages)) {
                // Для браузера всегда конвертируем GCM→ECB
                convertMessagesForBrowser($messages);
            }
            // === ДОБАВЬТЕ ЗДЕСЬ ===
            if (!empty($messages) && is_array($messages)) {
                convertMessagesForBrowser($messages);
                    ...
                    if ($message_id > 0) {
                        ...
                        $message = Wo_GetGroupMessages(array(
                            ...
                            'group_id' => $_POST['group_id']
                                                ...
                                                // === ДОБАВЬТЕ ЗДЕСЬ ===
                        if (!empty($message) && is_array($message)) {
                            convertMessagesForBrowser($message);
                        }
                        // === КОНЕЦ ДОБАВЛЕНИЯ ===
